...
1.  The slow (but more "complete" and fully supported) way, which requires the following:
    ...
    *   [SUSE Cloud 4 installation media](https://download.suse.com/FIXME) (again, you only need SUSE-CLOUD-4-x86_64-GM-DVD1.iso)
...
2.  The quick way (which is currently only supported on a best effort
    ...
    *   a specially constructed `SUSE-CLOUD-4-DEPS` `.iso`
        which contains the minimal set of packages which SUSE Cloud
        4 requires from SLES 11 SP3 and SLE 11 HAE SP3 including the
        latest updates, and
    *   an `.iso` of the latest (unreleased) development build of
        SUSE Cloud 4, which contains the latest updates.
...
*   `/mnt/sles-11-sp3`: the `SUSE-CLOUD-4-DEPS` `.iso`
*   `/mnt/suse-cloud-4`: the `.iso` of the latest development build of SUSE Cloud 4
...
Once you have the `.vmdk` built, do:
    ...
    cd ../../vagrant/cloud4-admin

and follow the instructions in
[the corresponding README](../../vagrant/cloud4-admin/README.md).
